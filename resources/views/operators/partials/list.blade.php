            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <div class="relative group" x-data="{ open: false }" @click.away="open = false">
                    @php
                        ...
                        $currentLabel = $sortFields[request('sort')] ?? __('Sort by');
                    @endphp
                    ...
                    <div x-show="open" 
                        ...
                        @foreach($sortFields as $field => $label)
                            <a href="{{ route('operators.index', array_merge(request()->all(), [
                               ...
                               class="flex items-center justify-between px-4 py-2.5 text-sm {{ request('sort') === $field ? 'bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-700 dark:text-gray-300' }} hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                {{ $label }}
